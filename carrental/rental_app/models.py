...
class Car(models.Model):
    ...
    category = models.CharField(
        max_length=50, choices=CATEGORIES, verbose_name=_("Category")
    )
    image = models.ImageField(
        upload_to="images", default="images/default_image.jpg", verbose_name=_("Image")
    )
    brand = models.CharField(max_length=50, verbose_name=_("Brand"))
    model = models.CharField(max_length=50, verbose_name=_("Model"))
    engine_type = models.CharField(
        max_length=20, choices=ENGINE_TYPES, verbose_name=_("Engine type")
    )
    ...
    gearbox_type = models.CharField(
        max_length=20, choices=GEARBOX_TYPES, verbose_name=_("Gearbox type")
    )
...
class UserData(AbstractUser):
    ...
    identity_document_no = models.CharField(
        max_length=20, verbose_name=_("Identity document number")
    )
    identity_document_type = models.CharField(
        max_length=20,
        choices=IDENTITY_DOCUMENT_TYPES,
...
class Address(models.Model):
    user = models.OneToOneField(
        UserData, on_delete=models.CASCADE, verbose_name=_("User")
    )
    ...
    appartment_no = models.CharField(
        max_length=10,
        blank=True,
        default="",
...
class Order(models.Model):
    ...
    customer = models.ForeignKey(
        UserData, on_delete=models.RESTRICT, verbose_name=_("Customer")
    )
    car = models.ForeignKey(Car, on_delete=models.RESTRICT, verbose_name=_("Car"))
    order_value = models.DecimalField(
        max_digits=10, decimal_places=2, verbose_name=_("Order value")
    )
    declared_order_duration = models.IntegerField(
        null=True, verbose_name=_("Declared rental duration")
    )
    ...
    deposit = models.DecimalField(
        max_digits=10, decimal_places=2, verbose_name=_("Deposit")
    )
    payment_method = models.CharField(
        max_length=20, choices=PAYMENT_METHODS, verbose_name=_("Payment method")
    )
